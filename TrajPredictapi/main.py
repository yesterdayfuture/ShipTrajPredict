...
# 轨迹共现判断 api
@app.post("/api/v1/co_traj", response_model=Union[R[Dict], R[str]], description="根据输入的船舶轨迹，判断是否在某个时空片段进行 同时出现 (输入数据的 key 值 必须包含 'MMSI','LAT','LON','timeUnix') 当前接口已更新，请使用v2版本")
...
# 轨迹共现判断 api
@app.post("/api/v2/co_traj", response_model=Union[R[Dict], R[str]],
          description="根据输入的船舶轨迹，判断是否在某个时空片段进行 同时出现 (输入数据的 key 值 必须包含 'MMSI','LAT','LON','timeUnix','Type','Belong')")
def co_traj2(data: Dict):
    # 获取 输入数据的 key 值
    clounms = list(data.keys())
    # 输入数据的 key 值 必须包含下方内容
    required_keys = ['MMSI', 'LAT', 'LON', 'timeUnix', 'Type', 'Belong']
    # 检查字典是否包含所有指定的键
    if not contains_keys(clounms, required_keys):
        return response_fail(data="输入数据不规范，字典缺少某些键")
    ...
    # 添加 类型、所属方、平均航速
    Co_result = Co_result.join(Co_result.apply(lambda row: getTypeAndBelongAndAvgSog(row, df), axis=1))
...
# 活动场景 分析 api
@app.post("/api/v1/traj_activity", response_model=Union[R[Dict], R[str]], description="根据输入的船舶轨迹，判断当前的轨迹所属 任务场景(输入数据的 key 值 必须包含 'MMSI','LAT','LON','timeUnix','lat_center','lon_center') 当前接口已更新，请使用v2版本")
